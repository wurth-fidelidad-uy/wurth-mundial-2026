...
def draw_card(equipo, capitan, score_raw, label_score, border_class=""):
    ...
    img_base64 = None
    # Probamos con el nombre del equipo para encontrar la foto
...
# ==============================================================================
# 🎨 ESTILOS CSS (DISEÑO ORIGINAL RECUPERADO)
# ==============================================================================
...
# --- 3. HEADER (LÓGICO Y LIMPIO) ---
# Aquí cargamos el logo de la copa. Asegúrate de que el archivo se llame logo_copa.png
...
st.markdown("<p style='text-align: center; font-size: 20px; color: #ccc;'>⚽ Tablero Oficial de Competencia</p>", unsafe_allow_html=True)
    ...
    with tab_m:
        if fase_grupos_finalizada:
            ...
            if not df_mundial.empty and (df_mundial['F3_Pedidos_Por_Dia'] > 0).any():
                ...
                with c1: draw_card(best['Equipo'], best['Capitan'], val, "Pedidos/Día", "highlight-gold")
                with c2: st.dataframe(df_mundial[['Equipo', 'Capitan', 'F3_Pedidos_Por_Dia', 'F2_TieBreak_Nuevos_Clientes']], hide_index=True, use_container_width=True)
            else:
