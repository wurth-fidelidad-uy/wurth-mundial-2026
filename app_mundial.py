...
<style>
    ...
    .fifa-card:hover {{ transform: scale(1.03); border-color: #cc0000; }}

    /* Foto Circular del Capitán */
    .captain-img {{
        width: 110px;
        height: 110px;
        border-radius: 50%;
    ...
    .pts-label {{ font-family: 'WuerthBook'; font-size: 13px; color: #aaa !important; text-transform: uppercase; }}
...
def get_image_base64(path):
    """Codifica la imagen local para incrustarla en el HTML."""
    try:
...
def draw_card(equipo, capitan, score_raw, border_class=""):
    # Buscar foto del capitán por nombre
    img_src = None
    if isinstance(capitan, str):
        for ext in [".png", ".jpg", ".jpeg", ".webp"]:
            path = f"{capitan.strip()}{ext}"
            if os.path.exists(path):
                img_src = get_image_base64(path)
                break
    
    # Imagen por defecto si no existe la foto
    if not img_src: 
...
if os.path.exists(archivo):
    ...
    with t1:
        st.markdown("### 📊 Resultado del Sorteo Inicial")
    ...
    with t2:
        st.markdown("### ⚔️ Posiciones de Fase de Grupos")
        cols = st.columns(4)
    ...
    st.error("⚠️ Archivo 'Planilla_Wurth_World_Cup_2026.xlsx' no encontrado en el repositorio.")
