...
# ==============================================================================
# 🎨 ESTILOS CSS (DISEÑO INSTITUCIONAL + MEDALLAS + IMÁGENES)
# ==============================================================================
...
<style>
    ...
    /* Estilo de las Imágenes de Supervisor */
    .supervisor-img {{
    ...
    .highlight-gold {{ border-color: #FFD700 !important; box-shadow: 0 0 20px rgba(255, 215, 0, 0.4) !important; }}
...
def draw_card(equipo, capitan, score_raw, label_score, border_class=""):
    ...
    img_base64 = None
    # Intenta buscar imagen con el nombre del equipo
...
try:
    ...
    st.error(f"⚠️ No se encontró: {archivo_excel}")
    datos_cargados = False

if datos_cargados:
    # 1. Ranking inicial y asignación de Grupos (Sorteo)
    ...
    # 2. Puntos Fase 2 (Máximo 14 por grupo: 3+2+4+5)
    df['Puntos_Fase2'] = 0
    ...
    for grupo in grupos_labels:
        ...
        for kpi, pts in reglas.items():
            ...
            if max_val > 0:
                ganador = df.loc[idx_g][df.loc[idx_g, kpi] == max_val].index[0]
                df.at[ganador, 'Puntos_Fase2'] += pts
        ...
        for i, g in enumerate(grupos_labels):
            with cols[i]:
                ...
                for _, row in df_g.iterrows():
                    draw_card(row['Equipo'], row['Capitan'], row['Puntos_Fase2'], "Puntos Totales", "highlight-gold" if row['Destino']=='Mundial' else "")
    ...
    with tabs[4]:
        ...
        st.markdown("<div style='text-align: center; margin-top: 50px;'><h2>📅 PARTIDOS Y PUNTOS</h2><a href='#' target='_blank' style='text-decoration:none;'><div style='display:inline-block; padding:20px 50px; background-color:#cc0000; border-radius:50px; border:2px solid white; color:white; font-family:WuerthExtra; font-size:24px; margin-top:20px;'>VER INFORMACIÓN 📊</div></a></div>", unsafe_allow_html=True)

    with tabs[5]:
        st.markdown("<div style='text-align: center; margin-top: 50px;'><h2>⚽ EQUIPOS Y FORMACIONES</h2><a href='http://www.wurth.com.uy' target='_blank' style='text-decoration:none;'><div style='display:inline-block; padding:20px 50px; background-color:#cc0000; border-radius:50px; border:2px solid white; color:white; font-family:WuerthExtra; font-size:24px; margin-top:20px;'>VER LA TARJETA DE CADA EQUIPO 🔗</div></a></div>", unsafe_allow_html=True)
